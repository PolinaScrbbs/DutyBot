...
async def get_group_by_id_with_students_and_applications(
...
async def get_group_by_title(session: AsyncSession, title: str) -> Group:
    try:
        result = await session.execute(select(Group).where(Group.title == title))
        group = result.scalars().one_or_none()
...
async def get_groups_without_application_from_user(
    ...
    try:
        ...
        groups_without_application = [
            group for group in groups if group.id not in groups_with_application_ids
        ]
...
async def get_students(session: AsyncSession, group_id: int) -> List[User]:
    try:
        result = await session.execute(
            select(User)
            .where(User.group_id == group_id, User.role == Role.STUDENT)
