...
async def accept_application(session: AsyncSession, application_id: int, sending_id: int) -> Tuple[Application, str]:
    try:
...
async def rejected_application(session: AsyncSession, application_id: int, sending_id: int) -> Tuple[Application, str]:
    try:
        application = await get_application_by_id(session, application_id)
        user = await get_user_by_id(session, sending_id)
