...
async def attendants(
    attendant_1_fullname: str, attendant_2_fullname: str
) -> InlineKeyboardMarkup:
    keyboard = InlineKeyboardBuilder()

    keyboard.row(
        InlineKeyboardButton(
            text=f"🔄 {attendant_1_fullname}", callback_data="replace_attendant_0"
        ),
        InlineKeyboardButton(
            text=f"🔄 {attendant_2_fullname}", callback_data="replace_attendant_1"
        ),
    )
    keyboard.row(
        InlineKeyboardButton(
            text="✅Назначить дежурных", callback_data="attendants_assign"
        )
    )
