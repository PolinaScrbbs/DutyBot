...
async def group_menu(application_count: Optional[str]):
    group_menu = InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [
                InlineKeyboardButton(
                    text=f"Заявки {application_count}", callback_data="grp_applications"
                )
            ],
...
async def inline_students(students):
    ...
    for j in range(0, len_range, 2):
        keyboard.row(
            InlineKeyboardButton(
                text=f"{students[j].username}",
                callback_data=f"st_{students[j].id}_{students[j].username}",
            ),
            InlineKeyboardButton(
                text=f"{students[j + 1].username}",
                callback_data=f"st_{students[j + 1].id}_{students[j + 1].username}",
            ),
        )
    ...
    if len(students) % 2 != 0:
        keyboard.row(
            InlineKeyboardButton(
                text=f"{students[-1].username}",
                callback_data=f"st_{students[-1].id}_{students[-1].username}",
            )
        )
...
async def inline_student(user_id):
    keyboard = InlineKeyboardBuilder()
    keyboard.row(
        InlineKeyboardButton(text="Выгнать", callback_data=f"delete_student_{user_id}")
    )
