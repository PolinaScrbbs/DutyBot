    ...
    try:
        ...
        if error_message:
            ...
            try:
                ...
                await message.answer(
                    f"❌*Ошибка:* {e}", parse_mode="Markdown", reply_markup=kb.start
                )

    except Exception as e:
        await message.answer(
            f"❌*Ошибка:* {str(e)}", parse_mode="Markdown", reply_markup=kb.start
        )
...
async def authorazation(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(
            f"*@{user.username}*, авторизация завершена ✌️", parse_mode="Markdown"
        )
        ...
        await message.answer(
            f"❌ Ошибка авторизации: {e}", parse_mode="Markdown", reply_markup=kb.start
        )
    finally:
        await session.close()
        await state.clear()
